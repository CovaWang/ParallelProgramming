  ...
  auto e = q.submit([&](sycl::handler &h) {
      h.parallel_for<class k_name_t>(
          sycl::nd_range<2>(global_ndrange, local_ndrange), [=](sycl::nd_item<2> index) {
              ...
              float subA[tileY][2] = { 0.0f };
              float subB[2][tileX] = { 0.0f };

               // core computation
               for (int k = 0; k < N; k+=2) { 8 // read data to register
                   for (int m = 0; m < tileY; m++) {
                      subA[m][0] = A[(row + m) * N + k];
                      subA[m][1] = A[(row + m) * N + k + 1];
                      }
                      for (int p = 0; p < tileX; p++) {
                      subB[0][p] = B[k * N + p + col];
                      subB[1][p] = B[(k + 1) * N + p + col];
                      }
                      for (int m = 0; m < tileY; m++) {
                      for (int p = 0; p < tileX; p++) {
                      sum[m][p] += subA[m][0] * subB[0][p] + subA[m][1] * subB[1][p];
                      }
                    }
                } //end of K
